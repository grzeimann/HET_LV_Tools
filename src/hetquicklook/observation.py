...
from .metadata import ObservationMetadata, metadata_from_members
...
class Observation:
    ...
    @classmethod
    def from_discovered(cls, discovered: DiscoveredObservation) -> "Observation":
        """Inventory one discovered archive and read its basic FITS metadata."""

        members = inventory_members(discovered)
        return cls(
            discovered=discovered,
            members=members,
            metadata=metadata_from_members(discovered, members),
        )


def load_observation(discovered: DiscoveredObservation) -> Observation:
    """Build an :class:`Observation` from one discovery result."""

    return Observation.from_discovered(discovered)
